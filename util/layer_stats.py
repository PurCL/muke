...
def layer_stats(
    ...
    def get_ds():
        ...
        if hasattr(lm_config, 'n_positions'):
            maxlen = lm_config.n_positions
        elif hasattr(lm_config, 'max_sequence_length'):
            maxlen = lm_config.max_sequence_length
        elif hasattr(lm_config, 'max_position_embeddings'):
            maxlen = lm_config.max_position_embeddings
        elif hasattr(lm_config,'seq_length'):
            maxlen = lm_config.seq_length
        else:
        ...
        if hasattr(model.config, 'model_type'):
            if 'mistral' in model.config.model_type:
                ...
                # qwen2.5: qwen.config.max_position_embeddings = 32768
                maxlen = 4096
            else:
                # llama3: llama.config.max_position_embeddings = 8192
                pass
    ...
    if hasattr(lm_config, 'n_positions'):
        npos = lm_config.n_positions
    elif hasattr(lm_config, 'max_sequence_length'):
        npos = lm_config.max_sequence_length
    elif hasattr(lm_config, 'max_position_embeddings'):
        npos = lm_config.max_position_embeddings
    elif hasattr(lm_config,'seq_length'):
        npos = lm_config.seq_length
    else:
        raise NotImplementedError
    ...
    with torch.no_grad():
        for batch_group in progress(loader, total=batch_count, desc="layer_stats"):
