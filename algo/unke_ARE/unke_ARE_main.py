...
def apply_unke_ARE_to_model(
    ...
    # Insert
    for i, layer in tqdm(enumerate(hparams.layers), total=len(hparams.layers), desc="Processing layers"):
        #print(f"\n\nLAYER {layer}\n")
        ...
        if hparams.model_name == 'Llama3-8B-Instruct':
            ...
            raise ValueError(f"Model {hparams.model_name} not supported")
...
def get_phi4_causal_mask(input_tensor, attention_mask):
    raise NotImplementedError

def get_gemma_causal_mask(input_tensor, attention_mask):
    raise NotImplementedError
