...
def compute_z(
    ...
    while start < len(target_ids):
        ...
        for it in tqdm(range(hparams.v_num_grad_steps), total=hparams.v_num_grad_steps, desc=f"Processing window {window_idx}"):
            opt.zero_grad()
            ...
            # print(
            # f"loss {np.round(loss, 3)} = {np.round(nll_loss, 3)}  + {np.round(weight_decay, 3)} "
            # #f"avg prob of [{cur_sen}] "
            # f"{avg_prob}"
            # )
            # tqdm.write(f'Window {window_idx}, Iteration {it}, Loss: {loss.item()}, NLL Loss: {nll_loss}, Weight Decay: {weight_decay}, Avg Prob: {avg_prob}')
