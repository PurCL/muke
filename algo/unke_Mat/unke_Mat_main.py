...
from .compute_z import compute_affinity_accumulate, compute_z_matryoshka
...
def apply_unke_Mat_to_model(
    ...
    for k, data in enumerate(batch_data):
        affinities = compute_affinity_accumulate(
            model,
            tok,
            data,
            z_layer,
        ...
        idxs_list, target_list = compute_z_matryoshka(
            model,
            tok,
            data,
            z_layer,
    ...
    for i, layer in enumerate(hparams.layers):
        ...
        for step in tqdm(range(hparams.optim_num_step), total=hparams.optim_num_step, desc=f"Processing layer {layer}"):
            optimizer.zero_grad()
            if hparams.model_name == 'Qwen2.5-7B-Instruct':
                ex_inputs_embeds = model.model.embed_tokens(ex_position_ids)
                ex_hidden_states = ex_inputs_embeds

                ex_position_embeddings = model.model.rotary_emb(ex_hidden_states, ex_position_ids)
                ...
                inputs_embeds = model.model.embed_tokens(input_position_ids)
                hidden_states = inputs_embeds
                
                position_embeddings = model.model.rotary_emb(hidden_states, input_position_ids)
                ...
                # Original unke loss before code refactor and forward pass fix.
                # loss = criterion(_layer(layer_in_ks,attention_mask=input_causal_mask,position_ids=input_position_ids,cache_position=input_cache_position)[0], layer_out_ks[:,-1])
