...
def compute_z(
    ...
    while start < len(target_ids):
        ...
        for it in tqdm(range(hparams.v_num_grad_steps), total=hparams.v_num_grad_steps, desc="Processing iterations"):
            opt.zero_grad()
            ...
            #     tqdm.write(
            # f"Window {window_idx} Iter {it} Loss {loss.item()} NLL {nll_loss} Weight {weight_decay} Avg {avg_prob}"
            # )
